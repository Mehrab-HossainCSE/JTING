...
import { PickingReconciliationRequest, LocalReconciliationRequest } from '../../../../../../core/models/receives/reconciliation/reconciliation';
...
export class RecPicking implements OnInit {
  ...
  uploadFile(): void {
    ...
    this.reconciliationService.pickingExcelFileReader(
      this.selectedFile,
      unitQtyStr,
      fromDateISO,
      toDateISO
  ...
  loadLocalData(): void {
    ...
    this.reconciliationService.getLocalPickingData(payload).subscribe({
      next: (res) => {
        this.isLoading.set(false);
        if (res.success && res.data) {
          if(res.data.length === 0) {
  ...
  saveData(): void {
    ...
    const listPicking = this.results().map(item => ({
      skucode: item.sku,
    ...
    const payload: PickingReconciliationRequest = {
    ...
    this.isLoading.set(true);
    this.reconciliationService.pickingReconciliation(payload).subscribe({
      next: (res) => {
        this.isLoading.set(false);
        if (res.success) {
