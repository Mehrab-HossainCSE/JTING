...
export class RecDelivery implements OnInit {
  ...
  uploadFile(): void {
    ...
    this.reconciliationService.deliveryExcelFileReader(
      this.selectedFile,
      unitQtyStr,
      fromDateISO,
      toDateISO
  ...
  loadLocalData(): void {
    ...
    this.reconciliationService.getLocalDeliveryData(payload).subscribe({
      next: (res) => {
        this.isLoading.set(false);
        if (res.success && res.data) {
          if(res.data.length === 0) {
  ...
  saveData(): void {
    ...
    const listDelivery = this.results().map(item => ({
      skucode: item.sku,
    ...
    const payload: DeliveryReconciliationRequest = {
