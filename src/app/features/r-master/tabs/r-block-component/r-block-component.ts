...
@Component({
  selector: 'app-r-block-component',
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule],
  templateUrl: './r-block-component.html',
  styleUrl: './r-block-component.scss',
})
export class RBlockComponent implements OnInit {
  ...
  private loadPermissionsFromStorage(): void {
    ...
    const currentUrl = '/r-block-setup';
    const blockMenu = masterMenu?.children?.find(
      (child) => child.url?.toLowerCase() === currentUrl || child.name?.toUpperCase() === 'R_BLOCKSETUP'
    );
  ...
  editBlock(item: Block): void {
    ...
    this.blockForm.get('blockId')?.disable();
  }
